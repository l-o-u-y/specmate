...
@Validator(RGModel)
export class NodeCycleValidator extends ElementValidatorBase<RGModel> {
    // List of all RGNodes of the model
    private nodes: RGNode[];
    // Maps the URL of a node to the node object.
    private nodeMap: {[nodeURL: string]: RGNode};
    // Maps the URL of an Node to all its outgoing connections.
    private outgoingConnections: RGConnection[][];
    // Set of already explored Nodes
    private closedSet: Set<RGNode>;
    ...
    private nodeComparitor(nodeA: RGNode, nodeB: RGNode) {
        //
    ...
    private depthFirstSearch(startNode: RGNode): Circle[] {
        /**
...
class TraceNode {
    ...
    public node: RGNode;
    public edges: RGConnection[];

    constructor(node: RGNode, outgoingEdges: RGConnection[]) {
        this.edgeCounter = 0;
        this.node = node;
