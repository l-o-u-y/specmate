...
public abstract class CEGFromRequirementGenerator implements ICEGFromRequirementGenerator {
	...
	private void handleNeitherAndNorOrInCause(Sentence sentence, JCas jCas, CEGModel model, LinkedList<CEGNode> nodes,
		...
		for (int i = 0; i < effects.length; i++) {
			...
			CEGNode effectNode = cegCreation.createNodeIfNotExist(model, splittedVariabelAndConditionEffect[0],
			...
			for (int j = 0; j < causes.length; j++) {
				...
				if (causesOr.length == 1) {
					...
					if (replaceNegation(causeNewOr) != null) {
						...
						CEGNode causeNodeOr = cegCreation.createNodeIfNotExist(model,
