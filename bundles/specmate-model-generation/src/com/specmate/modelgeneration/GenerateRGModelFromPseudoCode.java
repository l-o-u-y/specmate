...
/**
 * This class transforms pseudo code into a RG. The pseudo code must conform
 * with the implemented XText grammar.
 */
public class GenerateRGModelFromPseudoCode implements IRGFromRequirementGenerator {

	/** Our factory to create the elements of the RG (nodes and connections). */
	RGCreation creation = new RGCreation();
	...
	public RGModel createModel(RGModel model, String text) throws SpecmateException {
		...
		return createRGModelForEachSection(rule, model, null, false);
	}

	/**
	 * This method creates a RG model for each "WENN" --> "DANN" --> "ENDE-WENN"
	 * clause.
	 */
	private RGModel createRGModelForEachSection(BusinessRuleNode rule, RGModel model, RGNode brNode,
			boolean nesting) {
		...
		if (brNode == null) {
			rgNodeBR = creation.createNode(model, "BR" + (brnum++), "", 0, 0, NodeType.AND);
		} else {
			/**
		...
		if (rootEffect instanceof BusinessRuleNode) {
			 ...
			 */
			RGNode rgNodeBRNested = creation.createNode(model, "BR" + (brnum++), "", 0, 0, NodeType.AND);
			...
			/** superior rule */
			if (nesting == false) {
				connectCausesToBR(rgNodeBR, singleOrCauses, andCausesGroups, model);
			} else {
				/** inferior rule */
				connectCausesToNestedBR(rgNodeBR, singleOrCauses, andCausesGroups, model);
			}
			/**
			 ...
			 */
			creation.createConnection(model, rgNodeBR, rgNodeBRNested, false);
			/**
			 ...
			 */
			createRGModelForEachSection((BusinessRuleNode) rootEffect, model, rgNodeBRNested, true);
		} else {
			/**
			...
			if (rootCause instanceof LiteralNode) {
				 ...
				 */
				RGNode rgNode = creation.createNode(model, ((LiteralNode) rootCause).getContent(), "", 0, 0,
						null);
				creation.createConnection(model, rgNode, rgNodeBR, false);
				/** Connect the rule node with all identified effects. */
				for (ObjectifNode effect : effects) {
					RGNode rgNodeEffect = creation.createNode(model, ((LiteralNode) effect).getContent(), "",
							0, 0, null);
					creation.createConnection(model, rgNodeBR, rgNodeEffect, false);
				}
				/** Connect the rule node with its alternative effects */
				for (ObjectifNode effect : alternativeEffects) {
					RGNode rgNodeEffect = creation.createNode(model, ((LiteralNode) effect).getContent(), "",
							0, 0, null);
					creation.createConnection(model, rgNodeBR, rgNodeEffect, true);
				}
			} else {
				/**
				...
				connectBRToEffects(rgNodeBR, effects, alternativeEffects, model);
			}
		}
		return model;
	}

	private void connectBRToEffects(RGNode rgNodeBR, List<ObjectifNode> effects,
			List<ObjectifNode> alternativeEffects, RGModel model) {
		/**
		 * Iterate over all effects and connect the business rule node with these
		 * effects.
		 */
		for (ObjectifNode effect : effects) {
			RGNode rgNode = creation.createNode(model, ((LiteralNode) effect).getContent(), "", 0, 0, null);
			creation.createConnection(model, rgNodeBR, rgNode, false);
		}
		for (ObjectifNode effect : alternativeEffects) {
			RGNode rgNode = creation.createNode(model, ((LiteralNode) effect).getContent(), "", 0, 0, null);
			creation.createConnection(model, rgNodeBR, rgNode, true);
		}
	}

	/** Implementation for non nested BRs */
	private void connectCausesToBR(RGNode rgNodeBR, List<ObjectifNode> singleOrCauses,
			List<List<ObjectifNode>> andCausesGroups, RGModel model) {
		for (List<ObjectifNode> andCauses : andCausesGroups) {
			RGNode intermediateNode = null;
			if (andCausesGroups.size() > 0 && singleOrCauses.size() != 0) {
				intermediateNode = creation.createNode(model, "Intermediate Node" + (intermediatenum++), "",
						0, 0, NodeType.AND);
				creation.createConnection(model, intermediateNode, rgNodeBR, false);
			}
			for (ObjectifNode cause : andCauses) {
				RGNode rgNode = creation.createNode(model, ((LiteralNode) cause).getContent(), "", 0, 0,
						null);
				/**
				...
				if (andCausesGroups.size() == 1 && singleOrCauses.size() == 0) {
					 ...
					 */
					creation.createConnection(model, rgNode, intermediateNode, false);
				}
			}
		}
		/**
		...
		for (ObjectifNode cause : singleOrCauses) {
			RGNode rgNode = creation.createNode(model, ((LiteralNode) cause).getContent(), "", 0, 0, null);
			creation.createConnection(model, rgNode, rgNodeBR, false);
		}
	}
	 ...
	 */
	private void connectCausesToNestedBR(RGNode rgNodeBR, List<ObjectifNode> singleOrCauses,
			List<List<ObjectifNode>> andCausesGroups, RGModel model) {
		RGNode intermediateNodeNested = null;
		if (singleOrCauses.size() > 1) {
			intermediateNodeNested = creation.createNode(model, "Intermediate Node" + (intermediatenum++), "",
					0, 0, NodeType.AND);
			creation.createConnection(model, intermediateNodeNested, rgNodeBR, false);
		}
		for (List<ObjectifNode> andCauses : andCausesGroups) {
			RGNode intermediateNode = null;
			if (andCausesGroups.size() > 0 && singleOrCauses.size() != 0) {
		...
		for (ObjectifNode cause : singleOrCauses) {
			...
			if (singleOrCauses.size() == 1) {
				...
				creation.createConnection(model, rgNode, intermediateNodeNested, false);
			}
		}
	}
